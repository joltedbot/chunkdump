...
use crate::wave::cue::{read_cue_chunk_fields, CueFields};
...
use crate::wave::junk::read_junk_chunk_fields;
use crate::wave::list::read_list_chunk_fields;
use crate::wave::resu::{read_resu_chunk_fields, ResUFields};
...
impl Wave {
    pub fn new(file_path: String, mut wave_file: File) -> Result<Self, Box<dyn Error>> {
        ...
        let mut junk_data: String = Default::default();
        let mut format_data: FmtFields = Default::default();
        let mut cue_data: CueFields = Default::default();
        let mut resu_data: ResUFields = Default::default();
        let mut list_data: Vec<(String, String)> = Default::default();
        ...
        loop {
            ...
            println!("Chunk - {:?}", next_chunkid);

            match next_chunkid.as_str() {
                JUNK_CHUNKID => junk_data = read_junk_chunk_fields(&mut wave_file)?,
                FMT_CHUNKID => format_data = read_fmt_chunk_fields(&mut wave_file)?,
                FACT_CHUNKID => {
                    number_of_samples_per_channel = read_fact_chunk_fields(&mut wave_file)?
                }
                DATA_CHUNKID => read_data_chunk_fields(&mut wave_file)?,
                CUE_CHUNKID => cue_data = read_cue_chunk_fields(&mut wave_file)?,
                RESU_CHUNKID => resu_data = read_resu_chunk_fields(&mut wave_file)?,
                LIST_CHUNKID => list_data = read_list_chunk_fields(&mut wave_file)?,
